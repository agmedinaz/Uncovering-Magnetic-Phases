...
def generador_de_datos(c_e,L):
  ...
  """ 
  La funcion generador_de_datos(c_e,L) nos devuelve una dupla de  
  vectores (train_images,train labels).
  
  train images contiene una cantidad c_e de redes de espines cada configuración magnética de tamaño LxL.
  
  Los labels en train_labels son los siguientes:
...
# constructor de redes convolucionales simples
def get_conv(L,features_dim=[32,64,64]):
    model = models.Sequential()
    
    model.add(layers.Conv2D(features_dim[0],(3,3), activation='relu', input_shape=(L,L,1))) 
    model.add(layers.MaxPooling2D((2,2)))

    model.add(layers.Conv2D(features_dim[1],(3,3), activation='relu')) 
    model.add(layers.MaxPooling2D((2,2)))
    
    model.add(layers.Conv2D(features_dim[2],(3,3), activation='relu'))
...
def get_convnet(L,filters = 3, conv_dim = [32, 64, 64], dense_dim=[64], output_dim=4):
    
    model = models.Sequential()
    model.add(layers.Conv2D(conv_dim[0], (filters, filters), activation = 'relu', input_shape = (L, L, 1))) 
    model.add( layers.MaxPooling2D( (2, 2) ) )

    for fdim in conv_dim[1:]:
        model.add(layers.Conv2D(fdim,(3,3), activation = 'relu'))
        model.add(layers.MaxPooling2D((2,2)))
        
    model.add(layers.Flatten())
    
    for dim in dense_dim:
